...
import { z } from "zod";
import { SYSTEMS, type SystemName } from "./data/systems.js";
import { PATTERNS, type PatternName } from "./data/patterns.js";
...
server.tool(
  ...
  async ({ system }) => {
    const info = SYSTEMS[system as SystemName];
    if (!info) {
      return { content: [{ type: "text" as const, text: `Unknown system: ${system}` }] };
    }
    return {
      content: [{
        type: "text" as const,
...
server.tool(
  ...
  async ({ pattern }) => {
    const info = PATTERNS[pattern as PatternName];
    if (!info) {
      return { content: [{ type: "text" as const, text: `Unknown pattern: ${pattern}` }] };
    }
    return {
      content: [{
        type: "text" as const,
...
server.tool(
  "analyze_behavior",
  "Perform a structured ASEKE analysis of a behavioral pattern, situation, or conflict. Returns a 5-step analysis: system identification, IS capture analysis, trajectory assessment, blind spot identification, and strategy generation. This is the primary analytical tool.",
  {
  ...
  async ({ description, context, timescale }) => {
    const systemList = Object.values(SYSTEMS).map(s =>
      `${s.name}: ${s.functionalRole.split('.')[0]}.`
    ).join('\n');

    const patternList = Object.values(PATTERNS).map(p =>
      `${p.displayName}: ${p.substrate}`
    ).join('\n');
...
server.tool(
  "match_patterns",
  "Given observed behavioral signals, return matching patterns from the library with rationale for each match.",
  {
  ...
  async ({ signals }) => {
    const matches: Array<{ pattern: string; score: number; rationale: string }> = [];
    const signalText = signals.join(' ').toLowerCase();

    for (const p of Object.values(PATTERNS)) {
      let score = 0;
      const reasons: string[] = [];
      const checkTerms = (terms: string[], weight: number, source: string) => {
...
server.tool(
  ...
  async ({ activeSystems, dominantSystem, timescale }) => {
    const bridges: Record<string, { rwa: string; sdo: string; caveat: string }> = {
      FEAR: {
